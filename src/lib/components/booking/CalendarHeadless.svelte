<script lang="ts">
	...
	let {
		...
		patientEmail = '',
		username = 'pat.war',
		eventSlug = 'pre-booking',
		mode = 'immediate', // 'immediate' or 'select'
	...
	async function handleAction() {
		...
		const italyTime = italyTimeFormatter.format(timeObj);

		if (mode === 'select') {
			const rawSlot = currentDaySlots.find((s) => (s.start || s.time) === selectedTime);
			onSelect({
		...
		try {
			const res = await fetch('/api/cal/booking', {
				...
				body: JSON.stringify({
					...
					attendee: {
						name: patientName || 'Paziente',
						email: patientEmail || 'paziente@example.com',
			...
			if (res.ok && (result.data?.paymentUid || result.paymentUid)) {
				const payUid = result.data?.paymentUid || result.paymentUid;
				window.location.href = `https://cal.com/payment/${payUid}`;
			} else if (res.ok && (result.data?.uid || result.uid)) {
				alert('Prenotazione effettuata con successo!');
			} else {
				console.error('Booking failed:', result);
				alert('Prenotazione fallita. Riprova più tardi.');
			}
		} catch (err) {
...
{#snippet daySnippet({ day, outsideMonth }: { day: DateValue; outsideMonth: boolean })}
	<div class="relative flex h-full w-full flex-col items-center justify-center">
		<CalendarUI.Day class="z-10" />
		{#if !outsideMonth && isDateAvailable(day)}
			<div class="absolute bottom-1 h-1.5 w-1.5 rounded-full bg-primary"></div>
		{/if}
	</div>
{/snippet}
	...
	>
		...
		>
			{#if isBooking}
				...
				></div>
			{/if}
			{mode === 'select' ? i18n.t.cal.proceed : i18n.t.cal.proceed_payment}
		</Button>
	</Card.Footer>
</Card.Root>
